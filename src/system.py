...
def create_banner_motd(baseurl, cookie_header, motd):
    url = baseurl + 'banner'
    # print(url)
    headers = {'cookie': cookie_header}
    data = {
        "motd_base64_encoded": motd.decode("utf-8")
    }
    # print(data)
    response = requests.put(url, verify=False, data=json.dumps(data), headers=headers)
    # print("response status is", response.status_code)
    if response.status_code == 200:
        print("Banner update successful for motd. ")
        return response.status_code
    else:
        print("Banner Update Failed for motd - " + str(response))

def create_banner_exec(baseurl, cookie_header, exec):
    url = baseurl + 'banner'
    # print(url)
    headers = {'cookie': cookie_header}
    data = {
        "exec_base64_encoded": exec.decode("utf-8")
    }
    # print(data)
    response = requests.put(url, verify=False, data=json.dumps(data), headers=headers)
    # print("response status is", response.status_code)
    if response.status_code == 200:
        print("Banner update successful for exec. ")
        return response.status_code
    else:
        print("Banner Update Failed for exec - " + str(response))

def get_banner(baseurl, cookie_header):
    url = baseurl + 'banner'
    # print(url)
    headers = {'cookie': cookie_header}
    response = requests.get(url, verify=False, headers=headers)
    ...
    if response.status_code == 200:
        ...
        print("Error getting banners" + str(response))
